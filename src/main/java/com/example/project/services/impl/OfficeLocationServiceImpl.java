...
public class OfficeLocationServiceImpl implements OfficeLocationService {
    ...
    @Override
    public ResponseTemplate<OfficeLocationDto> updateOfficeLocation(Long id,
        OfficeLocationDto officeLocationDto) {
        OfficeLocation officeLocation = officeLocationRepository
            .findById(id)
            .orElseThrow(()->new ResourceNotFoundException("Office Location not found with id: %s" .formatted(id)));
